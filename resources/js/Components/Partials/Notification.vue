                ...
                <div v-if="visible" class="max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden">
                    <div class="p-4">
                        <div class="flex items-start">
                            <div class="flex-shrink-0" v-if="notification.success">
                                <check-circle-icon class="h-6 w-6 text-green-400" />
                            </div>
                            <div class="flex-shrink-0" v-if="notification.error">
                                <exclamation-circle-icon class="h-6 w-6 text-red-400" />
                            </div>
                            <div class="flex-shrink-0" v-if="notification.warning">
                                <exclamation-icon class="h-6 w-6 text-orange-400" />
                            </div>
                            <div class="ml-3 w-0 flex-1 pt-0.5" v-if="notification.success">
                            ...
                            <div class="ml-3 w-0 flex-1 pt-0.5" v-if="notification.error">
                                <p class="text-sm font-medium text-gray-900">
                                    Error!
                                </p>
                                <p class="mt-1 text-sm text-gray-500">
                            ...
                            <div class="ml-3 w-0 flex-1 pt-0.5" v-if="notification.warning">
                                <p class="text-sm font-medium text-gray-900">
                                    Warning!
                                </p>
                                <p class="mt-1 text-sm text-gray-500">
                            ...
                            <div class="ml-4 flex-shrink-0 flex">
                                <button @click="visible = false" class="bg-white rounded-md inline-flex text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
